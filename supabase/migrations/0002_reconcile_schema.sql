-- Workout Tracker — reconcile schema (idempotent).
-- Safe to run in full on any state: creates missing tables, (re)applies RLS
-- policies, (re)installs the new-user trigger, and backfills profile rows for
-- users who signed up before the trigger existed.
-- Run in Supabase Dashboard → SQL Editor → paste → Run. Report any error.
...
drop policy if exists "plans_self"           on public.plans;
drop policy if exists "plan_days_self"       on public.plan_days;
drop policy if exists "plan_exercises_self"  on public.plan_exercises;
...
-- Backfill profile rows for existing users (e.g. you) ------------------------
